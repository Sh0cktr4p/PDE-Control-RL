...
class BurgerEnv(gym.Env):
	...
	def continuous_double_size_with_time_space(self):
		return gym.spaces.Box(-np.inf, np.inf, shape=(2 * np.prod(list(self.size)) + 1,), dtype=np.float32)

	def zero_goal_obs(self):
		return np.real(np.zeros_like(self.state.velocity).reshape(-1))

	def random_goal_obs(self):
		return np.real(Burger(Domain(self.size), math.randn(levels=[0, 0, self.value_velocity_scale]), viscosity=0.2).velocity.reshape(-1))
	...
	def reachable_goal_obs(self):
		...
		return np.real(goal_obs.velocity.reshape(-1))
	...
	def weighted_mse_forces_reward(self, mse_old, mse_new, forces):
		force_strength = np.sum(forces ** 2)
		return -(2 * mse_new + force_strength)
	...
	def current_goal_time_obs(self):
		return np.concatenate((self.current_and_goal_obs(), [self.step_index / self.ep_len]), axis=0)
	...
	def get_obs(self):
		...
		elif self.observation_space == self.continuous_double_size_with_time_space():
			return self.current_goal_time_obs()
		else:
			raise NotImplementedError()
	...
	def plot_to_file(self):
		...
		path = os.path.join(self.image_dir, '%s_%04d.png' % ('Velocity%04i' % self.episode_index, self.step_index))
		plt.savefig(path)
		plt.close()
		...
		if self.step_index == self.ep_len - 1:
			images = []
			filenames = [os.path.join(self.image_dir, '%s_%04d.png' % ('Velocity%04i' % self.episode_index, i)) for i in range(self.ep_len)]
			for filename in filenames:
				images.append(imageio.imread(filename))
			output_file = os.path.join(self.image_dir, '%s.gif' % ('Velocity%04i' % self.episode_index))
