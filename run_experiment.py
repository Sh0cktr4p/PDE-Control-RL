...
def run_experiment(sim_name='burger', key='00', epochs=500, save_freq=50, label=''):
	...
	ac_kwargs = dict(hidden_sizes=[[4, 8, 16],[60, 50]], activation=torch.nn.ReLU, network=actor_critic.CNN)
	...
	ppo_pytorch(env_fn, ac_kwargs=ac_kwargs, steps_per_epoch=3200, epochs=epochs, logger_kwargs=logger_kwargs, 
			pi_lr=0.9e-4, vf_lr=3e-4, gamma=0.96, save_freq=save_freq, actor_critic=actor_critic.MLPActorCritic)
	toc = time.time()
...
run_experiment('navier', '16', 1000, 50, label='san')